    ...
    <div id={id} className="relative w-full h-32 md:h-48 bg-dark overflow-hidden flex items-center mt-12 mb-8">
      ...
      <div className="absolute inset-0 opacity-20 pointer-events-none">
        <div className="absolute inset-0" style={{ 
          backgroundImage: 'radial-gradient(circle at 10px 10px, #E81E25 1px, transparent 0)',
          backgroundSize: '40px 40px'
        }} />
      </div>

      <div className="max-w-7xl mx-auto px-4 w-full flex items-center gap-6 relative z-10">
        <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-primary flex items-center justify-center text-white flex-shrink-0">
          <Plus className="w-6 h-6 md:w-8 md:h-8" />
        </div>
        <h2 className="text-4xl md:text-6xl lg:text-8xl font-display text-primary leading-tight pt-2 uppercase">
          {title}
        </h2>
